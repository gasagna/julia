...
const MATCH_CONTEXTS = Ptr{Cvoid}[C_NULL]

_tid() = Int(ccall(:jl_threadid, Int16, ())+1)
_nth() = Int(unsafe_load(cglobal(:jl_n_threads, Cint)))

function match_context()
    return MATCH_CONTEXTS[_tid()]
end

function __init__()
    resize!(MATCH_CONTEXTS, _nth())
    for i = 1:length(MATCH_CONTEXTS)
        MATCH_CONTEXTS[i] = create_match_context()
    end
end
